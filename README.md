...
Owns trackingdb. Append-only tracking timeline built by consuming shipment.* events.
...
```bash
npm ci
npm run dev        # tsx watch, listens on 3004
...
`ghcr.io/echelon-luxe/logitrack-tracking-service:<git-sha>`.
